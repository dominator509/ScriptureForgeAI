...
export const AuthPanel: React.FC = () => {
  ...
  const submit = async (mode: 'login' | 'register') => {
    ...
    try {
      ...
      setSession(session.token, session.refresh_token, session.user_id);
      setStatus('Signed in');
    } catch (err) {
  ...
  return (
    <section className="bg-white border rounded p-4 space-y-3">
      ...
      <input className="w-full border rounded p-2" placeholder="Organization ID for registration" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} />
