...
func loadConfig() (*Config, *PlatformException) {
	...
	if allowedOriginsStr == "" {
		allowedOriginsStr = "http://localhost:3000,https://scriptureforgeai.com,https://*.scriptureforgeai.com"
	}
...
func setupRoutes(dbpool *pgxpool.Pool, vectorDB ai.VectorDB, redisClient *redis.Client, allowedOrigins []string) *http.ServeMux {
	mux := http.ServeMux{}
...
func main() {
	...
	router := setupRoutes(dbpool, vectorClient, rdb, cfg.AllowedOrigins)
	server := &http.Server{
		Addr:    ":" + cfg.Port,
		Handler: router,
	}
