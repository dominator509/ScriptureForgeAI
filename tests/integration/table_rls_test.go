...
func TestTenantRLSCoversAllTenantTables(t *testing.T) {
	...
	setTenantForTest(ctx, t, db, tenantOrgA, func(ctx context.Context, tx pgx.Tx) {
		...
		requireRLSWriteDenied(t, ctx, tx, `INSERT INTO ai_request_logs (id, organization_id, user_id, prompt, prompt_length, status) VALUES ($1, $2, $3, '[redacted]', 12, 'failed')`, tenantWriteProbe, tenantOrgB, tenantUserB)
...
func assertSameTenantWritesPassAllTables(ctx context.Context, t *testing.T, db *pgxpool.Pool) {
	...
	setTenantForTest(ctx, t, db, tenantWriteProbe, func(ctx context.Context, tx pgx.Tx) {
		...
		mustExecTenant(t, ctx, tx, `INSERT INTO ai_request_logs (id, organization_id, user_id, prompt, prompt_length, status) VALUES ($1, $2, $3, '[redacted]', 11, 'succeeded')`, tenantWriteAI, tenantWriteProbe, tenantWriteUser)
...
func seedTableRLSFixtures(ctx context.Context, t *testing.T, db *pgxpool.Pool) {
	...
	setTenantForTest(ctx, t, db, tenantOrgA, func(ctx context.Context, tx pgx.Tx) {
		...
		mustExecTenant(t, ctx, tx, `INSERT INTO ai_request_logs (id, organization_id, user_id, prompt, prompt_length, status) VALUES ($1, $2, $3, '[redacted]', 15, 'succeeded')`, tenantAILogA, tenantOrgA, tenantUserA)
	...
	setTenantForTest(ctx, t, db, tenantOrgB, func(ctx context.Context, tx pgx.Tx) {
		...
		mustExecTenant(t, ctx, tx, `INSERT INTO ai_request_logs (id, organization_id, user_id, prompt, prompt_length, status) VALUES ($1, $2, $3, '[redacted]', 15, 'succeeded')`, tenantAILogB, tenantOrgB, tenantUserB)
