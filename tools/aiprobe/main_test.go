...
func TestRunEmitsAIEvidenceWhenArtifactsPass(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/generation":
			_, _ = w.Write([]byte(`/api/v1/ai/generate/study returned 200 generated_curriculum with [Genesis 1:1]`))
		case "/degradation":
			_, _ = w.Write([]byte("timeout degradation returned 503 AI_ORCHESTRATION_ENGINE_FAULT"))
		case "/citation":
			_, _ = w.Write([]byte("no-citation rejected; hallucinated citation rejected; verified citation accepted"))
		case "/audit":
			_, _ = w.Write([]byte("ai_request_logs succeeded failed citation_trails verified"))
		default:
...
func TestRunFailsWhenProviderArtifactLeaksAPIKey(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/degradation":
			_, _ = w.Write([]byte("timeout degradation 503 AI_ORCHESTRATION_ENGINE_FAULT"))
		case "/citation":
...
func TestRunFailsWhenAuditArtifactMissingCitationTrails(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/citation":
			_, _ = w.Write([]byte("no-citation rejected hallucinated citation verified citation"))
		case "/audit":
			_, _ = w.Write([]byte("ai_request_logs succeeded failed verified"))
		}
	}))
	defer server.Close()
