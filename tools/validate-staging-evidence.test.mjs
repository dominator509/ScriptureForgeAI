...
test('validateManifest strict release rejects Terraform apply evidence without zero-destroy proof', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary.replace(
    'terraform-staging-apply-or-approval staging artifact deployment approval approved DEPLOY-TF-001 change_ticket=PLATFORM-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 distinct_terraform_artifacts=true',
    'terraform-staging-apply-or-approval staging artifact Apply complete Resources: release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 distinct_terraform_artifacts=true',
  );
...
test('validateManifest strict release rejects Terraform init and plan segments without release binding', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    ...
    .replace(
      'terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567',
...
test('validateManifest strict release rejects Terraform plan proof borrowed from backend segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      ...
      'terraform-remote-backend-init Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 staging artifact terraform',
    )
    .replace(
      'terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567',
...
test('validateManifest strict release rejects Terraform apply proof borrowed from plan segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      ...
      'terraform-staging-plan deployment approval approved DEPLOY-TF-001 change_ticket=PLATFORM-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 distinct_terraform_artifacts=true staging artifact Terraform',
    )
    .replace(
      'terraform-staging-apply-or-approval staging artifact deployment approval approved DEPLOY-TF-001 change_ticket=PLATFORM-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 distinct_terraform_artifacts=true',
...
test('validateManifest strict release rejects TLS evidence for a different release candidate', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replaceAll('release_candidate=0123456789abcdef0123456789abcdef01234567', 'release_candidate=oldsha')
    .replaceAll('service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567', 'service_version=scriptureforge-api:oldsha');
...
test('validateManifest strict release rejects Zoom duplicate proof without structured delivery ID on duplicate segment', () => {
  ...
  const item = manifest.items.find((candidate) => candidate.id === 'EXT-ZOOM-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace('zoom-duplicate-webhook-idempotency staging artifact duplicate x-zm-trackingid delivery_id=zm-delivery-123 delivery id', 'zoom-duplicate-webhook-idempotency staging artifact duplicate x-zm-trackingid delivery id')
...
test('validateManifest strict release rejects Zoom duplicate proof without tracking header marker', () => {
  ...
  const item = manifest.items.find((candidate) => candidate.id === 'EXT-ZOOM-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace('zoom-duplicate-webhook-idempotency staging artifact duplicate x-zm-trackingid delivery_id=zm-delivery-123', 'zoom-duplicate-webhook-idempotency staging artifact duplicate delivery_id=zm-delivery-123')
    .replace('zoom-meeting-room-mapping staging artifact meeting_external_id=zoom-123', 'zoom-meeting-room-mapping x-zm-trackingid staging artifact meeting_external_id=zoom-123');
...
test('validateManifest strict release rejects mobile staging config proof borrowed from EAS segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      'mobile-eas-or-device-run staging artifact eas build finished android ios native device installed app release channel staging expo profile staging',
      'mobile-eas-or-device-run staging artifact EXPO_PUBLIC_API_BASE_URL EXPO_PUBLIC_WS_BASE_URL EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging https:// wss:// staging eas build finished android ios native device installed app release channel staging expo profile staging',
    )
    .replace(
      'mobile-staging-config staging artifact EXPO_PUBLIC_API_BASE_URL EXPO_PUBLIC_WS_BASE_URL EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging https:// wss:// staging',
...
test('validateManifest strict release rejects DB user privilege proof borrowed outside scoped segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      'database-scoped-user staging artifact connected as scriptureforge_app superuser=false bypassrls=false createrole=false createdb=false privileged_operation_denied=true app_grants_verified=true app_grant_tables=9 app_grants=SELECT,INSERT,UPDATE,DELETE',
      'database-scoped-user staging artifact connected as scriptureforge_app; scoped-role-summary superuser=false bypassrls=false createrole=false createdb=false privileged_operation_denied=true app_grants_verified=true app_grant_tables=9 app_grants=SELECT,INSERT,UPDATE,DELETE',
    );
...
test('validateManifest strict release rejects degradation proof borrowed from rollout segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      ...
      'rollback-rollout-artifact AI Zoom degradation fallback AI_ORCHESTRATION_ENGINE_FAULT offline://in-person non-AI routes healthy zoom circuit open distinct_rollback_artifacts=true staging artifact rollout',
    )
    .replace(
      'degradation-drill-artifact staging artifact AI Zoom degradation fallback AI_ORCHESTRATION_ENGINE_FAULT offline://in-person non-AI routes healthy zoom circuit open distinct_rollback_artifacts=true release_candidate=',
...
function passedEvidenceFor(id) {
  ...
  return {
    ...
    result_summary: id === 'SRC-CI-001'
      ...
      : id === 'DEPLOY-TF-001'
        ? 'DEPLOY-TF-001 deploymentprobe passed: terraform-remote-backend-init staging artifact terraform s3 backend bucket key encrypt=true dynamodb_table successfully initialized release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; terraform-staging-apply-or-approval staging artifact deployment approval approved DEPLOY-TF-001 change_ticket=PLATFORM-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 distinct_terraform_artifacts=true'
      : id === 'DEPLOY-TLS-001'
        ? tlsSummary('0123456789abcdef0123456789abcdef01234567')
      : id === 'DEPLOY-K8S-001'
        ? 'DEPLOY-K8S-001 deploymentprobe passed: kubernetes-rollout-status staging artifact namespace staging deployment scriptureforge-api scriptureforge-web scriptureforge-rust-engine successfully rolled out ready available release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; kubernetes-workload-resources staging artifact namespace staging deployment service ingress hpa pdb ready available targets minavailable readinessProbe livenessProbe rollingUpdate maxUnavailable=0 minReplicas maxReplicas tls SecretProviderClass image scriptureforge-api@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa scriptureforge-web@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb scriptureforge-rust-engine@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 scriptureforge-api scriptureforge-web scriptureforge-rust-engine distinct_kubernetes_artifacts=true'
      : id === 'EXT-ZOOM-001'
        ? 'EXT-ZOOM-001 zoomprobe passed: zoom-oauth-readiness staging artifact oauth account_credentials status ok release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-meeting-create-or-fallback staging artifact meeting join_url zoom.us release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-timeout-circuit-fallback staging artifact timeout provider timeout circuit open circuit_open_fallback fallback offline://in-person provider_timeout=true circuit_open=true offline_fallback=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-webhook-signature-delivery staging artifact webhook signature x-zm-signature=v0=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa x-zm-request-timestamp=1710000000 stale replay 401 invalid signed 200 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-webhook-url-validation staging artifact endpoint.url_validation plain_token=zoom-plain-123 encrypted_token=zoom-encrypted-456 validation_response=200 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-duplicate-webhook-idempotency staging artifact duplicate x-zm-trackingid delivery_id=zm-delivery-123 delivery id same Zoom event idempotent 200 single state mutation no duplicate side effects release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; zoom-meeting-room-mapping staging artifact meeting_external_id=zoom-123 live_rooms internal_room_id=room-abc redis room state mapped unknown meeting ignored no external meeting id fallback distinct_zoom_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'EXT-AI-001'
        ? 'EXT-AI-001 aiprobe passed: ai-provider-config staging artifact AI_PROVIDER AI_CHAT_MODEL AI_CHAT_ENDPOINT AI_HTTP_TIMEOUT_MS AI_MAX_RETRIES OPENAI_API_KEY redacted configured AI_PROVIDER=openai AI_CHAT_MODEL=gpt-staging AI_CHAT_ENDPOINT=https://api.openai.com/v1/chat/completions AI_HTTP_TIMEOUT_MS=3500 AI_MAX_RETRIES=1 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; ai-generation-route staging artifact /api/v1/ai/generate/study authenticated JWT claims organization_id=org-staging user_id=user-staging request_id=req-123 200 generated_curriculum [Genesis 1:1] release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; ai-timeout-degradation staging artifact provider timeout degradation retry exhausted 503 fail closed AI_ORCHESTRATION_ENGINE_FAULT provider_timeout=true retry_exhausted=true fail_closed=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; ai-citation-verification staging artifact no-citation rejected hallucinated citation rejected verified citation accepted citation_trails citation_id=cite-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; ai-audit-persistence staging artifact ai_request_logs citation_trails organization_id=org-staging user_id=user-staging request_id=req-123 citation_id=cite-123 succeeded failed verified tenant rls cross-tenant hidden distinct_ai_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'OBS-OTEL-001'
        ? 'OBS-OTEL-001 observabilityprobe passed: collector-otlp-config staging artifact receivers otlp 4317 4318 exporters service release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; api-prometheus-metrics staging artifact scriptureforge_http_requests_total scriptureforge_http_request_duration_seconds_sum scriptureforge_http_requests_total{ status= websocket_active_connections_count scriptureforge_dependency_operations_total{dependency="websocket",operation="room_broadcast",status="dropped" ai_inference_duration_seconds_sum ai_inference_duration_seconds_count scriptureforge_dependency_operations_total{dependency="rust_engine",operation="vector_search",status="success" scriptureforge_dependency_operation_duration_seconds_sum{dependency="rust_engine",operation="vector_search",status="success" release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; rust-prometheus-metrics staging artifact scriptureforge_rust_engine_embedding_requests_total scriptureforge_rust_engine_embedding_failures_total scriptureforge_rust_engine_vector_search_requests_total scriptureforge_rust_engine_vector_search_failures_total release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; trace-backend-search staging artifact trace_id=11112222333344445555666677778888 scriptureforge-api scriptureforge-rust-engine route=/api/v1/ai/generate/study method=POST release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; log-backend-trace-correlation staging artifact trace_id=11112222333344445555666677778888 trace_id scriptureforge-api scriptureforge-rust-engine route=/api/v1/ai/generate/study method=POST service_version deployment_environment tenant_id=org-staging user_id=user-staging role=admin distinct_otel_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'OBS-ALERT-001'
        ? 'OBS-ALERT-001 observabilityprobe passed: dashboard-import staging artifact ScriptureForge scriptureforge_http_requests_total scriptureforge_http_request_duration_seconds_sum websocket_active_connections_count room_broadcast ai_inference_duration_seconds scriptureforge_rust_engine_ trace_id release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; alert-rules-loaded staging artifact ScriptureForgeHighErrorRate ScriptureForgeTrafficAbsent ScriptureForgeAuthFailureSpike ScriptureForgeAbuseLimitSpike ScriptureForgeRouteLatencyElevated ScriptureForgeDependencyFailures ScriptureForgeAIInferenceLatencyElevated ScriptureForgeJournalWriteFailures ScriptureForgeRoomStreamFailures ScriptureForgeRoomBroadcastDrops ScriptureForgeRustEngineFailures scriptureforge_http_requests_total scriptureforge_dependency_operations_total ai_inference_duration_seconds release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; alert-delivery-status staging artifact success alertname=ScriptureForgeHighErrorRate receiver=staging-release delivery_id=am-delivery-123 delivered test alert alertmanager release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; telemetry-retention-policy staging artifact retention 30 days trace logs metrics distinct_alert_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'RUST-GRPC-001'
        ? 'RUST-GRPC-001 rustprobe passed: rust-grpc-health staging artifact grpc health scriptureforge.engine.ScriptureEngine SERVING release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-rust-engine:0123456789abcdef0123456789abcdef01234567 deployment_environment=staging; rust-metrics staging artifact scriptureforge_rust_engine_embedding_requests_total scriptureforge_rust_engine_embedding_failures_total scriptureforge_rust_engine_vector_search_requests_total scriptureforge_rust_engine_vector_search_failures_total Prometheus metrics rust_metrics_samples_verified=true rust_embedding_requests_positive=true rust_vector_search_requests_positive=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-rust-engine:0123456789abcdef0123456789abcdef01234567 deployment_environment=staging embedding_requests=1 vector_search_requests=1; api-rust-integration-metrics staging artifact Go API rust_engine vector_search success scriptureforge_dependency_operations_total scriptureforge_dependency_operation_duration_seconds_sum api_rust_metrics_samples_verified=true distinct_metrics_targets=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-rust-engine:0123456789abcdef0123456789abcdef01234567 deployment_environment=staging api_rust_vector_search_ops=1 api_rust_vector_search_seconds=0.042'
      : id === 'CLIENT-MOBILE-001'
        ? 'CLIENT-MOBILE-001 mobileprobe passed: mobile-eas-or-device-run staging artifact eas build finished android ios native device installed app release channel staging expo profile staging platforms=android,ios release_channel=staging expo_profile=staging distinct_mobile_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-mobile:0123456789abcdef0123456789abcdef01234567; mobile-native-crypto-smoke staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip unique_iv=true unique IV tamper rejected associated data wrong associated data rejected associated_data_salt_id=journal:self-test:server-derived-salt associated_data_salt_version=1 non-extractable provider-bound key fallback-derived key rejected key disposed disposed handle rejected passphrase wiped passphrase buffer zeroized salt wiped salt buffer zeroized plaintext cleared plaintext buffer zeroized distinct_mobile_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-mobile:0123456789abcdef0123456789abcdef01234567; mobile-staging-config staging artifact EXPO_PUBLIC_API_BASE_URL EXPO_PUBLIC_WS_BASE_URL EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging https:// wss:// staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai distinct_mobile_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-mobile:0123456789abcdef0123456789abcdef01234567'
      : id === 'CLIENT-WEB-001'
        ? webClientSummary('0123456789abcdef0123456789abcdef01234567')
      : id === 'DATA-RLS-001'
        ? tenantRLSSummary('0123456789abcdef0123456789abcdef01234567')
      : id === 'SEC-SECRETS-001'
        ? 'SEC-SECRETS-001 securityprobe passed: irsa-service-account staging artifact namespace=staging service_account=scriptureforge-api role_arn=arn:aws:iam::123456789012:role/scriptureforge-app-secrets eks.amazonaws.com/role-arn scriptureforge trust policy sts:AssumeRoleWithWebIdentity release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; secret-provider-class staging artifact namespace=staging service_account=scriptureforge-api role_arn=arn:aws:iam::123456789012:role/scriptureforge-app-secrets SecretProviderClass secrets-store.csi.k8s.io provider aws objects objectName objectType secretsmanager objectAlias jmesPath secretObjects type Opaque DATABASE_URL JWT_SECRET_KEY OPENAI_API_KEY ZOOM_WEBHOOK_SECRET_TOKEN release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; synced-secret-metadata-redacted staging artifact namespace=staging scriptureforge-runtime-secrets type Opaque DATABASE_URL JWT_SECRET_KEY OPENAI_API_KEY ZOOM_WEBHOOK_SECRET_TOKEN redacted stringData absent managed by secrets-store.csi.k8s.io ownerReferences secrets-store.csi.k8s.io/managed=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; iam-secrets-policy staging artifact role_arn=arn:aws:iam::123456789012:role/scriptureforge-app-secrets secretsmanager:GetSecretValue secretsmanager:DescribeSecret arn:aws:secretsmanager: scoped resource no wildcard resources release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; scoped-secrets-access-test staging artifact namespace=staging service_account=scriptureforge-api role_arn=arn:aws:iam::123456789012:role/scriptureforge-app-secrets allowed configured secret denied unscoped secret AccessDenied distinct_secret_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'SEC-DBUSER-001'
        ? 'SEC-DBUSER-001 securityprobe passed: database-scoped-user staging artifact connected as scriptureforge_app superuser=false bypassrls=false createrole=false createdb=false privileged_operation_denied=true app_grants_verified=true app_grant_tables=9 app_grants=SELECT,INSERT,UPDATE,DELETE release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'DR-ROLLBACK-001'
        ? 'DR-ROLLBACK-001 resilienceprobe passed: api-ready-before-rollback staging artifact ready service_version deployment_environment pre_rollback_version release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; rollback-rollout-artifact staging artifact rollout undo revision previous_revision target_revision scriptureforge-api successfully rolled out release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; api-ready-after-rollback staging artifact ready service_version deployment_environment post_rollback_version rolled_back_from rolled_back_to release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; degradation-drill-artifact staging artifact AI Zoom degradation fallback AI_ORCHESTRATION_ENGINE_FAULT offline://in-person non-AI routes healthy zoom circuit open distinct_rollback_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'DR-BACKUP-001'
        ? 'DR-BACKUP-001 resilienceprobe passed: backup-snapshot-artifact staging artifact snapshot snapshot_id=snap-123 available encrypted kms retention automated backup source cluster rpo_minutes=15 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; restore-drill-artifact staging artifact restore restore_job_id=restore-456 available staging restored endpoint source snapshot_id=snap-123 checksum isolated restore rto_minutes=30 restore_duration_minutes=18 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; restored-database-smoke staging artifact smoke passed restored database tenant journal auth RLS migration version no plaintext journal distinct_backup_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567'
      : id === 'ABUSE-LIMIT-001'
        ? abuseLimitSummary('0123456789abcdef0123456789abcdef01234567')
      : id === 'PERF-HTTP-001'
        ? 'PERF-HTTP-001 profile=staging_http min_rps=5000 max_p99_ms=200 production_target_rps=5000 production_target_p99_ms=200 production_min_duration_ms=60000 observed_rps=5200 observed_p99_ms=180 duration_ms=60000 duration_ms>=60000 threshold_pass=true http_replica_count=2 dependency_postgres_p99_ms=32 dependency_redis_p99_ms=18 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567; verified markers: http_replica_artifact_verified, http_replica_count=2, dependency_telemetry_artifact_verified, dependency_latency_artifact_verified=true, dependency_postgres_p99_ms=32, dependency_redis_p99_ms=18, http_distinct_artifacts=true'
      : id === 'PERF-WS-001'
        ? 'PERF-WS-001 staging artifact profile=staging_websocket min_rps=500 max_p99_ms=200 production_target_rps=500 production_target_p99_ms=200 production_min_duration_ms=60000 production_min_ws_events=30000 duration_ms=60000 duration_ms>=60000 observed_rps=620 observed_p99_ms=140 ws_expected_events=30000 threshold_pass=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 ws_origin=https://web.staging.scriptureforge.ai ws_room_id=room-1 ws_authenticated=true ws_polling_latest_sequence=30000 ws_sequence_contiguous=true ws_replica_count=2 room_broadcast_drops=0; verified markers: staging artifact, ws_replica_artifact_url=https://artifacts.staging.scriptureforge.ai/load/ws-replicas.txt, ws_replica_artifact_verified, ws_replica_count=2, ws_reconnect_artifact_url=https://artifacts.staging.scriptureforge.ai/load/ws-reconnect.txt, ws_reconnect_artifact_verified, ws_reconnect_sequence_continues=true, ws_polling_artifact_url=https://artifacts.staging.scriptureforge.ai/load/ws-polling.txt, ws_polling_artifact_verified, ws_polling_artifact_latest_sequence_validated=true, ws_polling_artifact_latest_sequence_matches_run=true, redis_telemetry_artifact_url=https://artifacts.staging.scriptureforge.ai/load/redis-telemetry.txt, redis_telemetry_artifact_verified, ws_distinct_artifacts=true, room_broadcast_drops=0'
      : id === 'DATA-REDIS-001'
        ? 'DATA-REDIS-001 staging artifact profile=staging_websocket release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 ws_room_id=room-1 production_min_ws_events=30000 ws_sequence_contiguous=true ws_expected_events=30000 ws_unique_sequences=30000 ws_min_sequence=1 ws_max_sequence=30000 ws_polling_latest_sequence=30000 ws_polling_artifact_url=https://artifacts.staging.scriptureforge.ai/load/ws-polling.txt redis_telemetry_artifact_url=https://artifacts.staging.scriptureforge.ai/load/redis-telemetry.txt; verified markers: staging artifact, ws_polling_artifact_url=https://artifacts.staging.scriptureforge.ai/load/ws-polling.txt, redis_telemetry_artifact_verified, ws_polling_artifact_latest_sequence_validated=true, ws_polling_artifact_latest_sequence_matches_run=true, ws_distinct_artifacts=true, room_broadcast_drops=0'
      : id === 'SEC-SIGNOFF-001'
