...
test('validateManifest strict release rejects Terraform init and plan segments without release binding', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    ...
    .replace(
      'terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role kms_key_id=alias/scriptureforge-terraform-state database_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/11111111-1111-4111-8111-111111111111 redis_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/22222222-2222-4222-8222-222222222222 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123',
...
test('validateManifest strict release rejects Terraform evidence without concrete KMS values', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'DEPLOY-TF-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace('kms_key_id=alias/scriptureforge-terraform-state', 'kms_key_id=')
    .replace('kms_key_id=alias/scriptureforge-terraform-state', 'kms_key_id')
    .replace('database_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/11111111-1111-4111-8111-111111111111', 'database_kms_key_arn')
    .replace('redis_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/22222222-2222-4222-8222-222222222222', 'redis_kms_key_arn');
...
test('validateManifest strict release rejects Terraform plan proof borrowed from backend segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      ...
      'terraform-remote-backend-init Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role kms_key_id=alias/scriptureforge-terraform-state database_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/11111111-1111-4111-8111-111111111111 redis_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/22222222-2222-4222-8222-222222222222 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123 staging artifact terraform',
    )
    .replace(
      'terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role kms_key_id=alias/scriptureforge-terraform-state database_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/11111111-1111-4111-8111-111111111111 redis_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/22222222-2222-4222-8222-222222222222 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123',
...
test('validateManifest strict release rejects Zoom webhook proof without structured outcome markers', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'EXT-ZOOM-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(' replay_rejected=true', '');

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /EXT-ZOOM-001 strict release evidence must include Zoom markers on zoom-webhook-signature-delivery/,
  );
});
...
test('validateManifest strict release rejects Rust health proof borrowed from metrics segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(
      'rust-grpc-health staging artifact grpc health scriptureforge.engine.ScriptureEngine SERVING grpc_transport_security=mTLS release_candidate=',
...
test('validateManifest strict release rejects mobile crypto prose without structured lifecycle markers', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'CLIENT-MOBILE-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace(' key_disposed=true', '')
    .replace(' disposed_handle_rejected=true', '')
    .replace(' passphrase_buffer_zeroized=true', '')
    .replace(' salt_buffer_zeroized=true', '')
    .replace(' plaintext_buffer_zeroized=true', '')
    .replace(
      'mobile-staging-config staging artifact EXPO_PUBLIC_API_BASE_URL',
      'mobile-staging-config staging artifact key_disposed=true disposed_handle_rejected=true passphrase_buffer_zeroized=true salt_buffer_zeroized=true plaintext_buffer_zeroized=true EXPO_PUBLIC_API_BASE_URL',
    );
...
test('validateManifest strict release rejects synced-secret proof borrowed from SecretProviderClass segment', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    ...
    .replace(
      'synced-secret-metadata-redacted staging artifact namespace=staging scriptureforge-runtime-secrets type Opaque DATABASE_URL JWT_SECRET_KEY OPENAI_API_KEY JOURNAL_SALT_SECRET MFA_ENCRYPTION_KEY ZOOM_WEBHOOK_SECRET_TOKEN redacted stringData absent managed by secrets-store.csi.k8s.io',
...
test('validateManifest strict release rejects synced-secret proof without ownership markers', () => {
  ...
  item.evidence[0].result_summary = item.evidence[0].result_summary
    ...
    .replace(
      'synced-secret-metadata-redacted staging artifact namespace=staging scriptureforge-runtime-secrets type Opaque DATABASE_URL JWT_SECRET_KEY OPENAI_API_KEY JOURNAL_SALT_SECRET MFA_ENCRYPTION_KEY ZOOM_WEBHOOK_SECRET_TOKEN redacted stringData absent managed by secrets-store.csi.k8s.io ownerReferences secrets-store.csi.k8s.io/managed=true',
...
test('validateManifest strict release rejects backup snapshot without KMS key ID', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'DR-BACKUP-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace('kms_key_id=alias/scriptureforge-rds-backups', 'kms_key_id=');

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /DR-BACKUP-001 strict release backup-snapshot-artifact segment must include kms_key_id=<id>/,
  );
});
...
test('validateManifest strict release rejects abuse config proof borrowed from rate-limit segment', () => {
  ...
  abuseItem.evidence[0].result_summary = abuseItem.evidence[0].result_summary
    .replace(
      'auth-rate-limit staging artifact 429 after 2 attempts repeated_attempts_verified=true Retry-After=60 X-RateLimit-Limit=1 X-RateLimit-Remaining=0 X-RateLimit-Reset=1782403200',
      'auth-rate-limit staging artifact ABUSE_LIMIT_AUTH_REQUESTS=2 ABUSE_LIMIT_AUTH_WINDOW_SECONDS=60 ABUSE_LIMIT_AUTH_ACCOUNT_REQUESTS=2 ABUSE_LIMIT_AUTH_ACCOUNT_WINDOW_SECONDS=60 ABUSE_LIMIT_AI_REQUESTS=2 ABUSE_LIMIT_JOURNAL_REQUESTS=2 ABUSE_LIMIT_ROOMS_REQUESTS=2 ABUSE_LIMIT_WEBSOCKET_REQUESTS=2 ABUSE_LIMIT_ZOOM_WEBHOOK_REQUESTS=2 ABUSE_LIMIT_ZOOM_WEBHOOK_WINDOW_SECONDS=60 ABUSE_LIMIT_MAX_BUCKETS=1000 TRUST_PROXY_HEADERS=true X-Forwarded-For X-Real-IP redacted distinct_abuse_artifacts=true 429 after 2 attempts repeated_attempts_verified=true Retry-After=60 X-RateLimit-Limit=1 X-RateLimit-Remaining=0 X-RateLimit-Reset=1782403200',
    )
    .replace(
...
test('validateManifest strict release rejects HTTP load evidence without structured threshold proof', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
...
test('validateManifest strict release rejects HTTP structured threshold proof with drifted RPS', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'PERF-HTTP-001');
  item.evidence[0].structured_report.http_load_threshold_proof.observed_rps = 5300;

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /PERF-HTTP-001 structured report observed_rps must match summary marker/,
  );
});
...
test('validateManifest strict release rejects WebSocket load evidence with incomplete sequence range', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'PERF-WS-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replace('ws_max_sequence=30000', 'ws_max_sequence=29999');

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /PERF-WS-001 strict release maximum sequence must equal expected events/,
  );
});
...
test('validateManifest strict release rejects WebSocket reconnect proof without sequence continuity marker', () => {
  ...
  const item = manifest.items.find((candidate) => candidate.id === 'PERF-WS-001');
  item.evidence[0].result_summary = item.evidence[0].result_summary
    .replaceAll('ws_reconnect_sequence_continues=true', 'ws_reconnect_sequence_continues=false');
...
test('validateManifest strict release rejects WebSocket load evidence without structured sequence proof', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  ...
  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /PERF-WS-001 strict release evidence must include exactly one structured websocket_redis_sequence_proof report/,
  );
});

test('validateManifest strict release rejects WebSocket structured sequence proof with drifted room binding', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'PERF-WS-001');
  item.evidence[0].structured_report.websocket_redis_sequence_proof.ws_room_id = 'room-drift';

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /PERF-WS-001 structured report ws_room_id must match summary marker/,
  );
});

test('validateManifest strict release rejects Redis structured sequence proof with drifted event count', () => {
  const manifest = baseManifest({
    releaseCandidate: '0123456789abcdef0123456789abcdef01234567',
    statusFor: (id) => id === 'SEC-SIGNOFF-001' ? 'accepted_risk' : 'passed',
  });
  const item = manifest.items.find((candidate) => candidate.id === 'DATA-REDIS-001');
  item.evidence[0].structured_report.websocket_redis_sequence_proof.ws_unique_sequences = 29999;

  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /DATA-REDIS-001 structured report ws_unique_sequences must match summary marker/,
  );
});
...
test('validateManifest strict release rejects mixed performance load run identities', () => {
  ...
  assert.throws(
    () => validateManifest(manifest, { strictRelease: true }),
    /DATA-REDIS-001 structured report load_run_id must match summary marker/,
  );
});
...
function passedEvidenceFor(id) {
  ...
  return {
    ...
    result_summary: id === 'SRC-CI-001'
      ...
      : id === 'DEPLOY-TF-001'
        ? 'DEPLOY-TF-001 deploymentprobe passed: terraform-remote-backend-init staging artifact terraform s3 backend bucket key encrypt=true kms_key_id=alias/scriptureforge-terraform-state versioning=enabled dynamodb_table successfully initialized release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; terraform-staging-plan staging artifact Terraform Plan: aws_eks_cluster aws_eks_node_group aws_rds_cluster aws_elasticache_replication_group aws_ecr_repository kubernetes_deployment kubernetes_ingress_v1 kubernetes_horizontal_pod_autoscaler_v2 kubernetes_pod_disruption_budget_v1 kubernetes_manifest aws_iam_role kms_key_id=alias/scriptureforge-terraform-state database_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/11111111-1111-4111-8111-111111111111 redis_kms_key_arn=arn:aws:kms:us-east-1:123456789012:key/22222222-2222-4222-8222-222222222222 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; terraform-staging-apply-or-approval staging artifact deployment approval approved DEPLOY-TF-001 change_ticket=PLATFORM-123 release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123 distinct_terraform_artifacts=true'
      : id === 'DEPLOY-TLS-001'
      ...
      : id === 'OBS-OTEL-001'
        ? 'OBS-OTEL-001 observabilityprobe passed: collector-otlp-config staging artifact receivers otlp 4317 4318 exporters service release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; api-prometheus-metrics staging artifact scriptureforge_http_requests_total scriptureforge_http_request_duration_seconds_sum scriptureforge_http_requests_total{ status= websocket_active_connections_count scriptureforge_dependency_operations_total{dependency="websocket",operation="room_broadcast",status="dropped" ai_inference_duration_seconds_sum ai_inference_duration_seconds_count scriptureforge_dependency_operations_total{dependency="rust_engine",operation="vector_search",status="success" scriptureforge_dependency_operation_duration_seconds_sum{dependency="rust_engine",operation="vector_search",status="success" api_metrics_samples_positive=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; rust-prometheus-metrics staging artifact scriptureforge_rust_engine_embedding_requests_total scriptureforge_rust_engine_embedding_failures_total scriptureforge_rust_engine_vector_search_requests_total scriptureforge_rust_engine_vector_search_failures_total rust_metrics_samples_positive=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; trace-backend-search staging artifact trace_id=11112222333344445555666677778888 scriptureforge-api scriptureforge-rust-engine route=/api/v1/ai/generate/study method=POST release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123; log-backend-trace-correlation staging artifact trace_id=11112222333344445555666677778888 trace_id scriptureforge-api scriptureforge-rust-engine route=/api/v1/ai/generate/study method=POST timestamp=2026-07-01T12:00:00Z severity=info service_version deployment_environment tenant_id=org-staging user_id=user-staging role=admin distinct_otel_artifacts=true release_candidate=0123456789abcdef0123456789abcdef01234567 service_version=scriptureforge-api:0123456789abcdef0123456789abcdef01234567 load_run_id=load-run-123'
      : id === 'OBS-ALERT-001'
