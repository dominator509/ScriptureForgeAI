...
func TestRunFailsWhenNativeCryptoOmitsUniqueIVProof(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 AES-GCM native smoke round-trip passed; unique IV; tamper rejected; associated data; wrong associated data rejected; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
		{
			name:       "false unique iv marker",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 AES-GCM native smoke round-trip passed; unique_iv=false; unique IV; tamper rejected; associated data; wrong associated data rejected; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestRunFailsWhenNativeCryptoLacksAssociatedDataProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; unique_iv=true; unique IV; tamper rejected; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoLacksAssociatedDataSaltProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; unique_iv=true; unique IV; tamper rejected; associated data; wrong associated data rejected; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
