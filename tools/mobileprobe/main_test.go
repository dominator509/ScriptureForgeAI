...
const mobileStructuredOutcomeMarkersText = " aes_gcm_roundtrip=true tamper_rejected=true associated_data_rejected=true"
...
var requiredMobileProbeSummaryMarkers = map[string][]string{
	...
	"mobile-native-crypto-smoke": {"staging artifact", "react-native-quick-crypto", "native provider", "native module loaded", "provider status react-native-quick-crypto", "native-required true", "mobile_build_id=mobile-build-123", "device_os=ios", "device_model=iphone15pro", "app_runtime=installed-staging-app", "installed staging app runtime", "AES-GCM", "round-trip", "aes_gcm_roundtrip=true", "unique_iv=true", "unique IV", "tamper rejected", "tamper_rejected=true", "associated data", "wrong associated data rejected", "associated_data_rejected=true", "associated_data_salt_id=", "associated_data_salt_version=", "non-extractable", "provider-bound key", "fallback-derived key rejected", "key disposed", "key_disposed=true", "disposed handle rejected", "disposed_handle_rejected=true", "revoked_key_rejected=true", "stale raw key rejected", "passphrase wiped", "passphrase buffer zeroized", "passphrase_buffer_zeroized=true", "salt wiped", "salt buffer zeroized", "salt_buffer_zeroized=true", "plaintext cleared", "plaintext buffer zeroized", "plaintext_buffer_zeroized=true", "release_candidate=" + mobileReleaseCandidate, "service_version=" + mobileServiceVersion, "load_run_id=" + mobileLoadRunID, "distinct_mobile_artifacts=true"},
...
func TestRunEmitsMobileEvidenceWhenArtifactsPass(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/crypto":
			_, _ = w.Write([]byte("staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 device_os=ios device_model=iphone15pro app_runtime=installed-staging-app installed staging app runtime AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
	...
	if cryptoProbe.AESGCMRoundTrip == nil || *cryptoProbe.AESGCMRoundTrip != true {
		t.Fatalf("native crypto probe did not expose structured aes_gcm_roundtrip=true: %+v", cryptoProbe)
	}
	if cryptoProbe.TamperRejected == nil || *cryptoProbe.TamperRejected != true {
		t.Fatalf("native crypto probe did not expose structured tamper_rejected=true: %+v", cryptoProbe)
	}
	if cryptoProbe.AssociatedDataRejected == nil || *cryptoProbe.AssociatedDataRejected != true {
		t.Fatalf("native crypto probe did not expose structured associated_data_rejected=true: %+v", cryptoProbe)
	}
	if cryptoProbe.UniqueIV == nil || *cryptoProbe.UniqueIV != true {
		t.Fatalf("native crypto probe did not expose structured unique_iv=true: %+v", cryptoProbe)
	}
	...
	if cryptoProbe.DeviceOS != "ios" || cryptoProbe.DeviceModel != "iphone15pro" || cryptoProbe.AppRuntime != "installed-staging-app" {
		t.Fatalf("native crypto probe did not expose structured installed app runtime binding: %+v", cryptoProbe)
	}
}
...
func TestRunFailsWhenMobileBuildIDsDoNotMatch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/crypto":
			_, _ = w.Write([]byte("staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-other device_os=ios device_model=iphone15pro app_runtime=installed-staging-app installed staging app runtime AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging artifact mobile_build_id=mobile-build-123 EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoOmitsExactProviderMarkers(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto native-required true native_required=true AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
		{
...
func TestRunFailsWhenNativeCryptoOmitsUniqueIVProof(t *testing.T) {
	for _, tc := range []struct {
		name       string
		cryptoText string
	}{
		{
			name:       "missing unique iv marker",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 AES-GCM native smoke round-trip passed; unique IV; tamper rejected; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
		{
			name:       "false unique iv marker",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=false; unique IV; tamper rejected; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			err := runWithClient(stagingMobileConfig(time.Second), &output, clientForHTTPServer(t, server))
			if err == nil {
				t.Fatalf("expected native crypto artifact without unique IV proof to fail:\n%s", output.String())
			}
			if !strings.Contains(output.String(), "mobile-native-crypto-smoke") {
				t.Fatalf("report missing crypto probe:\n%s", output.String())
			}
...
func TestRunFailsWhenNativeCryptoOmitsStructuredOutcomeProof(t *testing.T) {
	baseCryptoText := "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 device_os=ios device_model=iphone15pro app_runtime=installed-staging-app installed staging app runtime AES-GCM native smoke round-trip passed; unique_iv=true; unique IV; tamper rejected; associated data; wrong associated data rejected; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"
	for _, tc := range []struct {
		name       string
		cryptoText string
	}{
		{
			name:       "missing structured outcomes",
			cryptoText: baseCryptoText + mobileReleaseMarkersText,
		},
		{
			name:       "false AES-GCM outcome",
			cryptoText: baseCryptoText + " aes_gcm_roundtrip=false tamper_rejected=true associated_data_rejected=true" + mobileReleaseMarkersText,
		},
		{
			name:       "false tamper outcome",
			cryptoText: baseCryptoText + " aes_gcm_roundtrip=true tamper_rejected=false associated_data_rejected=true" + mobileReleaseMarkersText,
		},
		{
			name:       "false associated-data outcome",
			cryptoText: baseCryptoText + " aes_gcm_roundtrip=true tamper_rejected=true associated_data_rejected=false" + mobileReleaseMarkersText,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if err == nil {
				t.Fatalf("expected native crypto artifact without structured outcome proof to fail:\n%s", output.String())
			}
			if !strings.Contains(output.String(), "mobile-native-crypto-smoke") {
				t.Fatalf("report missing crypto probe:\n%s", output.String())
			}
...
func TestRunFailsWhenNativeCryptoOmitsConcreteAssociatedDataSaltValues(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:       "empty salt id",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
		{
			name:       "empty salt version",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
		{
			name:       "zero salt version",
			cryptoText: "staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM native smoke round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=0; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true" + mobileReleaseMarkersText,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestRunFailsWhenNativeCryptoUsesNodeShim(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true using node:webcrypto"))
		case "/config":
...
func TestRunFailsWhenNativeCryptoUsesGenericNodeCrypto(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true via Node crypto"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoArtifactUsesRemoteDebug(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("staging artifact EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, mobile_build_id=mobile-build-123 platforms=android,ios release_channel=staging expo_profile=staging" + mobileReleaseMarkersText))
		case "/crypto":
			_, _ = w.Write([]byte("staging artifact runJournalCryptoSelfTest react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true mobile_build_id=mobile-build-123 AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true; remote debug" + mobileReleaseMarkersText))
		case "/config":
			_, _ = w.Write([]byte("staging artifact mobile_build_id=mobile-build-123 EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging" + mobileReleaseMarkersText))
		}
	}))
...
func TestRunFailsWhenNativeCryptoLacksKeyLifecycleProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoLifecycleProofIsOnlyProse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
	...
	err := runWithClient(stagingMobileConfig(time.Second), &output, clientForHTTPServer(t, server))
	if err == nil {
		t.Fatalf("expected prose-only lifecycle proof to fail:\n%s", output.String())
	}
	if !strings.Contains(output.String(), "mobile-native-crypto-smoke") {
		t.Fatalf("report missing crypto probe:\n%s", output.String())
	}
...
func TestRunFailsWhenNativeCryptoLacksAssociatedDataProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoLacksAssociatedDataSaltProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoLacksProviderBindingProof(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded native-required true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoUsesFallbackWebCrypto(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true via fallback WebCrypto"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoUsesJavaScriptFallback(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true through JavaScript fallback"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func TestRunFailsWhenNativeCryptoUsesBrowserWebCryptoFallback(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/eas":
			_, _ = w.Write([]byte("EAS build finished successfully for android and ios native device validation with installed app, release channel staging, expo profile staging, platforms=android,ios release_channel=staging expo_profile=staging, platforms=android,ios release_channel=staging expo_profile=staging"))
		case "/crypto":
			_, _ = w.Write([]byte("react-native-quick-crypto native provider native module loaded provider status react-native-quick-crypto provider=react-native-quick-crypto native-required true native_required=true AES-GCM round-trip passed; aes_gcm_roundtrip=true; unique_iv=true; unique IV; tamper rejected; tamper_rejected=true; associated data; wrong associated data rejected; associated_data_rejected=true; associated_data_salt_id=journal:self-test:server-derived-salt; associated_data_salt_version=1; non-extractable key verified; provider-bound key; fallback-derived key rejected; key disposed; key_disposed=true; disposed handle rejected; disposed_handle_rejected=true; revoked_key_rejected=true; stale raw key rejected; passphrase wiped; passphrase buffer zeroized; passphrase_buffer_zeroized=true; salt wiped; salt buffer zeroized; salt_buffer_zeroized=true; plaintext cleared; plaintext buffer zeroized; plaintext_buffer_zeroized=true through browser WebCrypto globalThis.crypto crypto.subtle shim"))
		case "/config":
			_, _ = w.Write([]byte("staging EXPO_PUBLIC_API_BASE_URL=https://api.staging.scriptureforge.ai EXPO_PUBLIC_WS_BASE_URL=wss://api.staging.scriptureforge.ai EXPO_PUBLIC_REQUIRE_NATIVE_CRYPTO=true EXPO_PUBLIC_DEPLOYMENT_ENVIRONMENT=staging"))
		}
	}))
...
func clientForHTTPServerWithReleaseMarkers(t *testing.T, server *httptest.Server, appendReleaseMarkers bool) *http.Client {
	...
	return &http.Client{
		...
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			_ = resp.Body.Close()
			markers := mobileReleaseMarkersText
			if strings.Contains(req.URL.Path, "/crypto") {
				markers += mobileStructuredOutcomeMarkersText
			}
			resp.Body = io.NopCloser(strings.NewReader(string(body) + markers))
