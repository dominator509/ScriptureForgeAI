...
const zoomSegmentMarkerRequirements = new Map([
  ['zoom-oauth-readiness', ['staging artifact', 'oauth', 'account_credentials', 'status', 'ok', 'release_candidate=', 'service_version=']],
  ['zoom-meeting-create-or-fallback', ['staging artifact', 'release_candidate=', 'service_version=']],
...
const aiSegmentMarkerRequirements = new Map([
  ['ai-provider-config', ['staging artifact', 'AI_PROVIDER', 'AI_CHAT_MODEL', 'AI_CHAT_ENDPOINT', 'AI_HTTP_TIMEOUT_MS', 'AI_MAX_RETRIES', 'OPENAI_API_KEY redacted', 'configured', 'release_candidate=', 'service_version=']],
  ['ai-generation-route', ['staging artifact', '/api/v1/ai/generate/study', 'authenticated', 'JWT claims', 'organization_id=', 'user_id=', 'request_id=', '200', 'generated_curriculum', '[Genesis 1:1]', 'release_candidate=', 'service_version=']],
  ['ai-timeout-degradation', ['staging artifact', 'provider timeout', 'degradation', 'retry exhausted', '503', 'fail closed', 'AI_ORCHESTRATION_ENGINE_FAULT', 'release_candidate=', 'service_version=']],
  ['ai-citation-verification', ['staging artifact', 'no-citation rejected', 'hallucinated citation rejected', 'verified citation accepted', 'citation_trails', 'citation_id=', 'release_candidate=', 'service_version=']],
  ['ai-audit-persistence', ['staging artifact', 'ai_request_logs', 'citation_trails', 'organization_id=', 'user_id=', 'request_id=', 'citation_id=', 'succeeded', 'failed', 'verified', 'tenant rls', 'cross-tenant hidden', 'distinct_ai_artifacts=true', 'release_candidate=', 'service_version=']],
]);
...
const httpPerformanceSegmentMarkerRequirements = new Map([
  ['PERF-HTTP-001', ['profile=staging_http', 'min_rps=5000', 'max_p99_ms=200', 'production_target_rps=5000', 'production_target_p99_ms=200', 'production_min_duration_ms=60000', 'duration_ms=', 'duration_ms>=60000', 'observed_rps=', 'observed_p99_ms=', 'threshold_pass=true', 'http_replica_count=', 'dependency_postgres_p99_ms=', 'dependency_redis_p99_ms=', 'release_candidate=', 'service_version=']],
...
const redisPerformanceSegmentMarkerRequirements = new Map([
  ['DATA-REDIS-001', ['staging artifact', 'profile=staging_websocket', 'release_candidate=', 'service_version=', 'ws_room_id=', 'ws_sequence_contiguous=true', 'production_min_ws_events=30000', 'ws_expected_events=', 'ws_unique_sequences=', 'ws_min_sequence=1', 'ws_max_sequence=', 'ws_polling_latest_sequence=', 'redis_telemetry_artifact_url=https://']],
...
const resilienceSegmentMarkerRequirements = new Map([
  ...
  ['api-ready-after-rollback', ['staging artifact', 'ready', 'service_version', 'deployment_environment', 'post_rollback_version', 'rolled_back_from', 'rolled_back_to', 'release_candidate=']],
  ['degradation-drill-artifact', ['staging artifact', 'AI', 'Zoom', 'degradation', 'fallback', 'AI_ORCHESTRATION_ENGINE_FAULT', 'offline://in-person', 'non-AI routes healthy', 'zoom circuit open', 'distinct_rollback_artifacts=true', 'release_candidate=', 'service_version=']],
  ['backup-snapshot-artifact', ['staging artifact', 'snapshot', 'snapshot_id', 'available', 'encrypted', 'kms', 'retention', 'automated backup', 'source cluster', 'rpo_minutes', 'release_candidate=', 'service_version=']],
  ['restore-drill-artifact', ['staging artifact', 'restore', 'restore_job_id', 'available', 'staging', 'restored endpoint', 'source snapshot_id', 'checksum', 'isolated restore', 'rto_minutes', 'restore_duration_minutes', 'release_candidate=', 'service_version=']],
  ['restored-database-smoke', ['staging artifact', 'smoke passed', 'restored database', 'tenant', 'journal', 'auth', 'RLS', 'migration version', 'no plaintext journal', 'distinct_backup_artifacts=true', 'release_candidate=', 'service_version=']],
]);
