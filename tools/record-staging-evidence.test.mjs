...
const securityProbeMarkerSummaries = {
  ...
  'secret-provider-class': 'got HTTP 200; verified markers: staging artifact, namespace=staging, service_account=scriptureforge-api, role_arn=arn:aws:iam::123456789012:role/scriptureforge-app-secrets, SecretProviderClass, secrets-store.csi.k8s.io, provider, aws, objects, objectName, objectType, secretsmanager, objectAlias, jmesPath, secretObjects, type, Opaque, DATABASE_URL, JWT_SECRET_KEY, OPENAI_API_KEY, JOURNAL_SALT_SECRET, MFA_ENCRYPTION_KEY, ZOOM_WEBHOOK_SECRET_TOKEN, release_candidate=abc123, service_version=scriptureforge-api:abc123 load_run_id=load-run-123',
  'synced-secret-metadata-redacted': 'got HTTP 200; verified markers: staging artifact, namespace=staging, scriptureforge-runtime-secrets, type, Opaque, DATABASE_URL, JWT_SECRET_KEY, OPENAI_API_KEY, JOURNAL_SALT_SECRET, MFA_ENCRYPTION_KEY, ZOOM_WEBHOOK_SECRET_TOKEN, redacted, stringData absent, managed by secrets-store.csi.k8s.io, ownerReferences, secrets-store.csi.k8s.io/managed=true, release_candidate=abc123, service_version=scriptureforge-api:abc123 load_run_id=load-run-123',
