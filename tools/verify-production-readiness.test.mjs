...
function strictEvidenceSummary(id, releaseCandidate = sha) {
  ...
  if (id === 'ABUSE-LIMIT-001') {
    ...
    return [
      ...
      'auth-account-rate-limit staging artifact 429 after 2 attempts repeated_attempts_verified=true Retry-After=60 X-RateLimit-Limit=1 X-RateLimit-Remaining=0 X-RateLimit-Reset=1782403200 account-scoped login rotating forwarded client IP',
      'auth-refresh-rate-limit staging artifact 429 after 2 attempts repeated_attempts_verified=true Retry-After=60 X-RateLimit-Limit=1 X-RateLimit-Remaining=0 X-RateLimit-Reset=1782403200 refresh token',
