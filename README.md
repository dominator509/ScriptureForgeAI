...
The recorder rejects `DATA-RLS-001` reports that target local/non-HTTPS APIs, omit the owner/blocked journal probes, do not return `201` for owner create, `200` for owner read, `404` for blocked direct read, `200` for blocked list, or do not include an HTTPS non-local database RLS artifact probe.
...
The recorder rejects `CLIENT-MOBILE-001` reports unless all three mobile probes pass and point at HTTPS non-local artifacts for EAS/native-device output, native AES-GCM crypto smoke, and staging API/WS configuration.
