...
- Terraform defines an IRSA-scoped workload service account, a least-privilege Secrets Manager read policy for those ARNs, and a Secrets Store CSI `SecretProviderClass` that syncs `DATABASE_URL`, `JWT_SECRET_KEY`, `OPENAI_API_KEY`, and Zoom credential environment variables into API/Rust pods from AWS Secrets Manager.
