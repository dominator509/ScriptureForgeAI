...
The 2026-09-01 repository security review identified and remediated a low-severity structured-log integrity issue in the Rust engine: hand-written JSON escaping now covers every JSON control character, with a focused regression test. The review also recorded the remaining live-provider, deployment, and staging evidence boundaries separately from local source verification.
...
- A 2026-09-01 mobile audit identified high-severity Browserslist advisories in the transitive Metro chain; a lockfile-only refresh to `browserslist 4.28.8` and current browser-data packages closed the finding without a major Expo/React Native change. The post-refresh mobile audit reports 0 vulnerabilities.
