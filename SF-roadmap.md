...
• Acceptance Criteria: The communication architecture handles thousands of simultaneous client synchronization actions over websocket paths while maintaining latency limits inside design tolerances. External orchestration adapters connect with target systems, safely handling API faults via fallback routes. Production WebSocket evidence must include authenticated WSS load, contiguous Redis sequence proof, reconnect behavior proof, and HTTP polling fallback proof against the same staged room state, with distinct artifacts for replica distribution, reconnect behavior, polling fallback, and Redis telemetry.
