...
- Current MFA enrollment bootstrap hardening (2026-08-27): unenrolled privileged users now receive a short-lived purpose-bound enrollment JWT after password verification, scoped by middleware to the MFA enrollment and verification routes and never paired with a refresh token. Web/mobile setup helpers keep it in memory, stage the encrypted seed, activate it only after a TOTP proof, and require a fresh normal login; current database role checks prevent stale privileged claims from changing MFA state.
