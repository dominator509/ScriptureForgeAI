...
- Current Zoom response-boundary hardening (2026-08-11): outbound Zoom OAuth, meeting, and status JSON responses are capped at 1 MiB before decoding; transient retries no longer drain unbounded bodies, raw provider error bodies are excluded from application errors, missing tokens/meeting fields fail closed, and malformed or oversized meeting responses use the existing offline fallback. Focused adapter tests cover oversized responses and missing access tokens; live Zoom evidence remains external under `EXT-ZOOM-001`.
