...
- Current security boundary hardening (2026-08-17): refresh-token family containment and current-MFA assurance prevent reuse or privilege elevation, AI generation requires moderator/author authorization and rejects malformed citation-like claims, Zoom retries reuse idempotency keys and validate provider meeting URLs/IDs, signed webhook dedupe is Redis-backed, journal clients clear identity-bound plaintext/key state, WebSocket ingress has event/byte budgets, and Rust vector search stays inside the tenant transaction. Focused and hosted gates pass; proxy identity trust, AI endpoint allowlisting, Rust/API secret separation review, and live staging evidence remain open.
