...
- Current security boundary hardening (2026-08-17): refresh-token family containment and current-MFA assurance prevent reuse or privilege elevation, AI generation requires moderator/author authorization and rejects malformed citation-like claims, AI chat/embedding calls require an exact provider-host allowlist before bearer credentials are sent, Zoom retries reuse idempotency keys and validate provider meeting URLs/IDs, signed webhook dedupe is Redis-backed, journal clients clear identity-bound plaintext/key state, WebSocket ingress has event/byte budgets, Rust vector search stays inside the tenant transaction, and trusted proxy headers require an allowlisted peer CIDR. Terraform now keeps Rust server TLS material out of the API SecretProviderClass and workload secret object. Focused validation covers these controls; live staging evidence remains open.
