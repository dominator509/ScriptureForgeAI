...
- Current CI hardening (2026-08-17): security workflow actions, the pgvector Postgres service image, and the gopls install now use immutable pins; workflow regression tests reject mutable action/image/tool references and legacy Node20 action pins.
- Current exact-release CI evidence (2026-08-17): commit `610efd0e50cb1f949e4a0cff2f8a66e5bbb83eab` passed the clean `34/34` local gate matrix at `2026-08-17T06:38:22Z` with ahead/behind `0/0` and hosted Security Pipeline Verification run `32001756732`; `tools/ciprobe` validated the release artifact as `SRC-CI-001`. Environment-specific staging-manifest ingestion and all deployed/staging evidence remain pending.
- Latest exact-release CI attempt (2026-08-27): push run `33071077208` for `006559ea68d0cd56679d54af0728e306ffb2b40f` failed before workflow steps (`runner_id=0`, `steps=[]`); hosted CI account/runner remediation remains external. Focused local deployment validation passed, while the full matrix rerun was blocked by denied Docker daemon access.
- Latest exact-release CI attempt (2026-08-27): push run `33146814553` and pull-request run `33146821720` for `0c63a86a32795f5692fd85d2bf1280ac8f8b2c43` failed before workflow steps (`runner_id=0`, `steps=[]`); hosted CI account/runner remediation remains external.
- Historical exact-release CI verification (2026-08-28): branch tip `24f6d7d9885ec357a569fc30216832c4d20bf9d5` passed push run `33159284084` and pull-request run `33159292139`, including PostgreSQL/RLS integration, client checks, Rust, Terraform, secret scanning, clean-source verification, and CI release-evidence upload.
- Historical exact-release CI verification (2026-08-28): branch tip `e07f423b519566277bf5f9d72681309c79d8cb00` passed Security Pipeline Verification push/PR run `33184095128`, including PostgreSQL/RLS integration, observability and Obsidian snapshot checks, client checks, Rust, Terraform, secret scanning, clean-source verification, and CI release-evidence upload.
- Historical clean local matrix (2026-08-28): exact SHA `7e52902ba92032e05bebf27b5293b6cb61c5a7ca` executed 35/35 gates with 34 passing at `2026-08-28T09:46:40Z`; `git_remote_refreshed=true`, the worktree was clean, and ahead/behind was 0/0. Only `rls-db-integration` failed because the local Docker server endpoint was unreachable. The fail-closed DB gate remains required and no local RLS proof claim is made from this run.
- Current exact-release CI verification (2026-08-29): latest behavior-changing hardening commit `b880c1886644fbc64a15214c70661efaaf5b27bf` passed hosted pull-request run `33260522160`, including the real PostgreSQL/RLS and CI release-evidence gates. Staging-manifest ingestion and deployed evidence remain pending.
- Current exact-release CI verification (2026-08-29): room-state fail-closed hardening commit `d7f8326a7faaee9e0e46727bb6a63f43c4847d31` passed hosted pull-request run `33261950730`, including the real PostgreSQL/RLS and CI release-evidence gates. Local Go test/race/vet evidence also passed; staging-manifest ingestion and deployed evidence remain pending.
- Current source hardening (2026-08-28): protected HTTP routes and refresh issuance enforce `users.sessions_revoked_at`; tenant email uniqueness uses `(organization_id, email)` with generic registration conflicts and dummy Argon2 verification for absent users; AI chat requests carry bounded `AI_MAX_OUTPUT_TOKENS`; API/Rust database egress is constrained to required `data_tier_cidrs`; and failed room provisioning cleanup uses a short context detached from client cancellation. Focused Go, Node, migration, and deployment-validator gates pass; staging/provider/CNI evidence remains external.
- Current local-gate resilience hardening (2026-08-28): the disposable Docker/RLS runner now checks Docker daemon readiness with `docker info` and applies bounded subprocess budgets, so denied or stalled Docker access produces a fast explicit environment blocker instead of hanging the full matrix. RLS remains fail-closed and still requires a real migrated database for proof.
- Current security boundary hardening (2026-08-28): refresh-token family containment commits replay revocation before denial, current-MFA assurance prevents reuse or privilege elevation, room creation requires moderator/admin authorization, AI generation requires moderator/author authorization and rejects malformed citation-like claims, AI chat/embedding calls require an exact provider-host allowlist before bearer credentials are sent, provider clients do not follow redirects, Zoom retries reuse idempotency keys and validate provider meeting URLs/IDs, signed webhook dedupe is Redis-backed, journal clients clear identity-bound plaintext/key state, WebSocket ingress has event/byte budgets, Rust vector search stays inside the tenant transaction, and trusted proxy headers require an allowlisted peer CIDR. Non-local environment names now fail closed for browser origins, HSTS, refresh-cookie Secure, and client endpoint/native-crypto configuration. Terraform keeps Rust server TLS material out of the API SecretProviderClass and workload secret object. Focused validation covers these controls; live staging evidence remains open.
...
- Current mobile session continuity hardening (2026-08-29): mobile normal sessions now persist only access/refresh identity fields through Expo SecureStore, serialize storage writes so logout cannot be overwritten by an older login write, refresh or clear persisted state before marking the app ready, and keep MFA enrollment tokens out of storage. Mobile smoke/typecheck gates cover the secure record, invalid-state cleanup, refresh bootstrap, and rejected-session cleanup; native-device and staging proof remain external.
- Current exact-release CI verification (2026-08-29): latest behavior-changing hardening commit `8e750f6391579079b4f9d5781890d4995fa79242` passed hosted Security Pipeline Verification pull-request run `33267057168`, including PostgreSQL/RLS, local gate runner, web/mobile, Rust, Terraform, secret scanning, source-control cleanliness, and CI release-evidence upload. `tools/ciprobe` now accepts the GitHub Actions artifact ZIP with scoped credential handling; environment-specific staging-manifest ingestion and deployed evidence remain pending.
- Current API metrics exposure hardening (2026-08-28): the Go API now protects `/metrics` with a constant-time bearer-token comparison in every non-local environment, fails closed with `503` when `METRICS_AUTH_TOKEN` is absent, and keeps explicit local/development/test behavior for local tooling. Terraform projects the API-only secret through Secrets Manager/CSI, deployment and observability validators guard the wiring, and Rust/staging probes accept `STAGING_METRICS_AUTH_TOKEN` for API scrape evidence. Live Prometheus secret delivery and protected staging scrape proof remain external.
...
- Current live-session revocation hardening (2026-08-28): logout now persists a tenant-scoped `users.sessions_revoked_at` cutoff after refresh-family revocation; default room WebSockets require issued-at JWTs and re-check both RLS membership and the cutoff during their heartbeat, closing streams after logout or membership removal. The ordered RLS runner discovers every checked-in up-migration, including `000006_auth_session_revocation`.
- Current AI audit prompt-redaction hardening (2026-08-28): generation audit rows now persist only a fixed `[redacted]` marker plus prompt byte length, status, errors, and citation trails; migration `000007_ai_prompt_redaction` irreversibly scrubs historical prompt bodies, and direct RLS fixtures follow the redacted contract. Local generation/audit and migration tests cover the boundary; provider-side retention and deployed audit evidence remain external under `EXT-AI-001`.
- Current AI source-label hardening (2026-08-29): the RAG verifier now accepts citations only when they exactly label a retrieved segment, and context assembly flattens source line breaks so citation-like text inside verse content cannot be treated as metadata. Focused and full Go gates pass; semantic claim-to-source validation, provider cost quotas, and staging/provider evidence remain external or product-defined boundaries.
- Current registration tenant-boundary hardening (2026-08-28): public registration no longer accepts a caller-selected `organization_id`; it creates a fresh organization inside the same RLS-scoped transaction, forces the initial role to `member`, and returns the generated tenant ID for subsequent login/session calls. Web/mobile registration callers now submit only a bounded workspace name; legacy route aliases share the same safe handler. Focused rejection, client-smoke, and disposable Postgres auth/session coverage protect the boundary.
- Current MFA assurance hardening (2026-08-28): refresh tokens persist an `mfa_verified_at` assurance timestamp, privileged refreshes require both an active factor and prior MFA proof, privilege elevation cannot turn a member refresh token into a privileged JWT without a fresh MFA challenge, and TOTP verification attempts are bounded by a hashed tenant/user abuse bucket shared across client IPs. Integration and focused handler coverage prove the elevation denial, verified-token assurance, and rotating-IP throttle behavior.
- Current MFA at-rest hardening (2026-08-17): TOTP seeds are stored as versioned AES-GCM envelopes using an API-only `MFA_ENCRYPTION_KEY`, with wrong-key, malformed-envelope, and plaintext-persistence tests; legacy plaintext rows fail closed and require re-enrollment. Migration `000005_mfa_legacy_plaintext_cleanup` now clears legacy plaintext values and disables those factors without attempting secret conversion; live execution against an existing deployment remains an operator gate. Terraform projects the key only into the API secret provider.
- Current migration gate hardening (2026-08-28): CI and the disposable RLS runner apply the complete ordered `migrations/*.up.sql` set, including the scripture reference uniqueness migration and the irreversible legacy-MFA cleanup, so integration databases cannot silently omit a checked-in migration.
- Current MFA enrollment bootstrap hardening (2026-08-27): unenrolled privileged users now receive a short-lived purpose-bound enrollment JWT after password verification, scoped by middleware to the MFA enrollment and verification routes and never paired with a refresh token. Web/mobile setup helpers keep it in memory, stage the encrypted seed, activate it only after a TOTP proof, consume the setup token once enabled, and require a fresh normal login; current database role checks prevent stale privileged claims from changing MFA state.
...
- Current EKS Secret protection hardening (2026-08-28): Terraform now requires `eks_secrets_kms_key_arn` and binds it to the EKS `encryption_config` for Kubernetes Secret envelope encryption. Deployment validation and its regression tests protect the variable, documented input, secret resource selection, and customer-managed key binding; staging KMS policy, rotation, and deployed behavior remain external.
- Current RDS credential-state hardening (2026-08-28): Aurora now manages the RDS master password in Secrets Manager with the customer-managed database KMS key; the Terraform skeleton accepts no root-password variable or tfvars value, and deployment validation rejects Terraform-managed `master_password`. Encrypted remote-state access, managed-secret rotation, and scoped application-user staging proof remain external.
- Current Kubernetes network-policy hardening (2026-08-28): the Terraform skeleton now applies namespace-wide default-deny ingress/egress and explicit API, Rust, and web rules for ALB ingress, in-cluster service calls, DNS, database/Redis, Prometheus, OTLP, and HTTPS provider traffic; no unrestricted pod egress rule is present. Local Terraform/validator coverage passes; live CNI enforcement and staging network-policy telemetry remain external.
- Current workload image hardening (2026-08-27): API and Rust Dockerfiles now build the current root entrypoints from the repository root, web has a production Next.js image definition, and the deployment validator rejects legacy in-memory or placeholder sleep images. Local definitions are validated; image publishing, immutable ECR digests, and staging rollout evidence remain external.
- Current legacy-runtime cleanup (2026-08-28): the unreachable nested in-memory `services/platform-engine` module and WAL implementation were removed; that directory now contains only the root-context API Dockerfile and a boundary note. Local Compose and deployment validation point only at the current PostgreSQL/Redis-backed API.
- Current recovery-tooling hardening (2026-08-28): disaster-recovery helpers now use checksummed PostgreSQL custom-format archives and explicit isolated-target restore confirmations; the superseded mock WAL report is historical and cannot support production claims. Staging backup/restore and rollback drills remain external under `DR-BACKUP-001` and `DR-ROLLBACK-001`.
- Current database transport hardening (2026-08-28): Go and Rust startup now fail closed in staging/production unless `DATABASE_URL` uses PostgreSQL `sslmode=require`, `verify-ca`, or `verify-full`; local development retains relaxed connection-string behavior. Focused Go/Rust tests and the deployment skeleton guard pass; live secret-content and database TLS evidence remain staging-owned.
...
- Current room credential hardening (2026-08-28): web and mobile room streams send the short-lived access JWT in the `scriptureforge-bearer` WebSocket subprotocol; validated JWTs must carry an expiration claim, and active sockets close at that expiry instead of outliving the access token. Backend middleware rejects `?ticket=` credentials and the room upgrader negotiates only the named protocol. Client reconnect/polling recovery remains covered locally; deployed token-expiry/reconnect evidence remains external.
...
- Current Zoom room provisioning wiring (2026-08-28): production room route construction injects the configured `MeetingAdapter` into `RoomHandler`; room creation provisions Zoom or its explicit offline fallback before persisting `meeting_provider`, `meeting_external_id`, and safe join metadata under the authenticated tenant transaction. Host `start_url` values are never persisted or returned to participants, and failed room initialization attempts terminate external meetings when possible. Focused mapping tests and the tenant integration gate cover adapter invocation and persisted identity; live provider and staging evidence remain external under `EXT-ZOOM-001`.
- Current Zoom missed-webhook reconciliation (2026-08-29): `GET /api/v1/rooms/active` checks mapped external meetings within a bounded two-second budget and deactivates exact tenant-scoped durable/Redis room state for terminal provider statuses. Provider errors preserve state for retry, and offline fallback rooms are not treated as external meetings. Focused room tests and the Postgres/RLS tenant path cover the local transition; scheduled multi-replica reconciliation and live provider evidence remain external under `EXT-ZOOM-001`.
- Current Redis room-state fail-closed hardening (2026-08-29): event sequencing, publication, active-state updates, participant duration, polling, and webhook idempotency now reject nil receiver or client wiring with typed room-state faults instead of dereferencing an unavailable Redis client. Focused, full, race, and vet gates pass; deployed Redis outage and multi-replica evidence remain external.
- Current Zoom webhook RLS mapping hardening (2026-08-29): signed webhook room lookup runs in a transaction-local context with a valid non-tenant sentinel `app.current_org_id`, `app.webhook_lookup_verified=true`, and the exact `meeting_external_id`; dedicated `live_rooms` SELECT and UPDATE policies permit only that verified exact-ID lookup and durable lifecycle update under FORCE RLS. `meeting.started` and `meeting.ended` synchronize `live_rooms.is_active` before Redis publication. Transient mapping/state failures return `503` without consuming the delivery ID so Zoom can retry; the DB-backed regression is part of the canonical RLS gate, while live webhook evidence remains external.
