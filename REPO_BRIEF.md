...
- API transport: the Go `http.Server` applies bounded read-header (5s), read (30s), write (30s), and idle (60s) deadlines plus a 1 MiB header cap by default; `HTTP_*` environment overrides are range-validated and Terraform projects the same validated settings, while upgraded WebSocket connections retain their handler-owned ping/read/write deadlines.
