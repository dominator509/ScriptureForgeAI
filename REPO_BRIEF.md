...
- Privileged users without an enrolled factor receive a short-lived `mfa_enrollment_token` challenge after password verification. That JWT is purpose-bound to `/api/v1/auth/mfa/enroll` and `/api/v1/auth/mfa/verify`, never gets a refresh token, and must be exchanged for a normal login after TOTP activation; web/mobile keep the setup token only in component memory.
