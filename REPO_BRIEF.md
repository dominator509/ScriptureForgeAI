...
- Worker/service: Rust `1.97.1`, edition 2021 + Tokio + Tonic gRPC.
...
- Browser sessions keep refresh tokens in an HttpOnly, SameSite=Strict `/api` cookie; access JWTs remain memory-only and reload through cookie-backed bootstrap. Mobile keeps the compatibility body-token flow, persists only its normal access/refresh session fields through Expo SecureStore, and refreshes or clears that record during app startup; MFA enrollment tokens remain memory-only. Both clients use short-lived access tokens with rotation. Login persists its first refresh token in the same tenant-scoped transaction as credential verification, avoiding nested pool acquisition under constrained capacity. Logout also records a tenant-scoped `users.sessions_revoked_at` cutoff; protected HTTP routes, refresh issuance, and active room sessions reject tokens issued at or before logout.
- User email uniqueness is tenant-scoped by the composite `(organization_id, email)` index from migration `000008_tenant_scoped_user_email`; public registration uses a generic conflict response, and missing-user login attempts perform bounded dummy Argon2id verification to reduce account-enumeration timing signals.
...
- Privileged TOTP seeds are persisted only as versioned AES-GCM envelopes under the API-only `MFA_ENCRYPTION_KEY`; missing or legacy plaintext seed material fails closed until the factor is re-enrolled. Staging/production startup also requires a strong MFA key distinct from `JWT_SECRET_KEY` and `JOURNAL_SALT_SECRET`. Migration `000005_mfa_legacy_plaintext_cleanup` clears legacy plaintext values and disables the factor irreversibly, forcing safe re-enrollment; live execution against an existing deployment remains an operator gate.
...
- Live rooms rely on authenticated membership + JWT claims plus allowed origin checks; room WebSocket JWTs travel in the `scriptureforge-bearer` subprotocol and query-string credentials are rejected. Validated JWTs require expiration and issued-at claims, active room sockets close at token expiry, and the default RLS path re-checks membership plus the user session-revocation cutoff on each heartbeat so logout or membership removal closes existing streams. Socket ingress uses a strict event envelope that rejects unknown fields, missing/null payloads, and client-supplied sequence values before an atomic Redis Lua sequence/latest-state append and pub/sub broadcast. Redis-backed room hubs subscribe per active room, suppress their own origin-tagged publication, and close subscriptions during shutdown; local hub broadcast remains the bounded fallback. Active room sockets also consume a Redis-backed leased semaphore across global, tenant, and user scopes, renew every 30 seconds, expire after two minutes without renewal, and fail closed with a sanitized `503` when Redis is unavailable. All room-state operations reject nil receiver/client wiring with typed room-state faults. Room creation returns a sanitized `503` and marks the durable room inactive under tenant RLS if Redis active-state initialization fails after the PostgreSQL commit, preventing an unusable room from being advertised as active.
- Zoom webhooks are HMAC/timestamp verified after a dedicated public `zoom_webhook` Redis fixed-window budget and before room mapping. Mapping runs under FORCE RLS with a transaction-local non-tenant sentinel plus `app.webhook_lookup_verified=true` and an exact `meeting_external_id` policy; `meeting.started` and `meeting.ended` update the mapped durable `live_rooms.is_active` row before Redis state publication through the verified update policy. `GET /api/v1/rooms/active` also performs a bounded two-second status reconciliation for mapped external meetings and deactivates exact tenant/meeting rows plus Redis state on terminal provider statuses; provider failures preserve state, and offline fallback rooms are skipped. Transient mapping/state failures return `503` without consuming the delivery ID so provider retries remain safe. Staging abuse proof requires `ABUSE_LIMIT_ZOOM_WEBHOOK_REQUESTS` and `ABUSE_LIMIT_ZOOM_WEBHOOK_WINDOW_SECONDS`.
...
- Go API `/metrics` requires `Authorization: Bearer <METRICS_AUTH_TOKEN>` outside explicit local/development/test modes and returns sanitized `503` when its non-local secret is missing; Terraform and staging probes keep the token API-only, while the Rust metrics listener remains a separate target.
...
- Terraform applies a namespace-wide default-deny Kubernetes `NetworkPolicy` and explicit API, Rust, and web ingress/egress rules for the declared service ports, DNS, database/Redis, Prometheus scraping, OTLP, and required HTTPS provider traffic. API/Rust database-port egress is limited to required `data_tier_cidrs`; unrestricted pod egress is not part of the skeleton. Live CNI enforcement and network-policy observation remain staging-owned.
...
- AI and Zoom paths fail closed/degrade with explicit typed errors and auditability; AI chat/embedding calls and decoded Zoom OAuth/meeting/status responses use bounded response bodies and retry policy, with request-body replay, cancellation-aware backoff, sanitized provider errors, UTF-8-safe bounded MapReduce chunks, and bounded MapReduce concurrency. The Zoom client refuses redirects before credentialed requests can reach an unexpected host. Zoom credentials are trimmed and blank values fail closed before OAuth transport; request timeout/retry budgets are environment-driven through `ZOOM_HTTP_TIMEOUT_MS` and `ZOOM_MAX_RETRIES`, clamped to finite ranges, and Terraform projects the same bounds into the API workload. Production room creation injects the Zoom `MeetingAdapter`, persists tenant-scoped provider/external meeting identity plus safe join metadata, and never persists or returns host start URLs. AI generation requires structurally complete DB, vector, verifier, LLM configuration/HTTP, and MapReduce dependencies plus audit persistence; its aggregate curriculum uses an 8 MiB response envelope and amortized assembly, while missing configuration, oversized output, or audit writes return typed `503` faults instead of serving unaudited/partial output. Direct RAG/LLM calls fail closed on incomplete wiring. AI provider keys, endpoints, and models are trimmed at construction, blank keys fail closed before chat or embedding transport, and the reducer model honors `AI_CHAT_MODEL` with the existing `gpt-4-turbo` fallback; reducer provider errors are redacted.
- AI audit rows now retain only a fixed `[redacted]` prompt marker, prompt byte length, status, errors, and citation trails. Migration `000007_ai_prompt_redaction` scrubs historical prompt bodies and has an intentionally irreversible rollback; no raw prompt content is persisted by new generation attempts. LLM chat requests include the bounded, environment-configurable `AI_MAX_OUTPUT_TOKENS` budget (2048 default, 8192 maximum).
...
- Strict database principal: staging/production API and Rust startup require `DATABASE_URL` to name the scoped `scriptureforge_app` runtime role; local development retains its disposable `postgres` setup. Live grants and non-superuser/bypass-RLS proof remain staging-owned.
...
- Terraform RDS security: Aurora manages the root password in Secrets Manager with the customer-managed database KMS key; the deployment skeleton intentionally has no root-password variable or tfvars value. Live remote-state access controls and managed-secret rotation evidence remain external.
- Abuse controls: production route wiring uses atomic Redis fixed-window request limits, including the unauthenticated public Zoom webhook `zoom_webhook` profile, and a Redis-backed leased semaphore for active room sockets, all shared across replicas. Remote identity registration is bounded with an overflow bucket, connection leases renew and expire after crashes, and unavailable Redis produces sanitized 503 fail-closed responses. Forwarded client headers are ignored by default; the Terraform skeleton keeps `TRUST_PROXY_HEADERS=false`, and enabling it requires verified ingress stripping/overwriting plus exact `TRUSTED_PROXY_CIDRS`. The process-local limiter is retained only for explicit nil-client local/test wiring.
...
- Client lifecycle: web and mobile API clients perform single-flight refresh-token rotation after expired access tokens, surface privileged MFA challenges as structured login results, apply server-verified auth roles as normalized in-memory UI hints, reset role state on session loss, reconnect room WebSockets with bounded backoff plus authenticated HTTP state polling fallback, and apply a configurable 1-120 second API request deadline (15 seconds by default) while preserving caller cancellation. Room creation/presenter controls are hidden for non-moderators, but server RBAC remains authoritative. Configured API/WebSocket base URLs are trimmed and trailing-slash normalized; strict environments reject credential-bearing, query-bearing, or fragment-bearing endpoints.
