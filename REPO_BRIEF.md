...
- Zoom webhooks are HMAC/timestamp verified after a dedicated public `zoom_webhook` Redis fixed-window budget and before room mapping. Mapping runs under FORCE RLS with a transaction-local non-tenant sentinel plus `app.webhook_lookup_verified=true` and an exact `meeting_external_id` policy; `meeting.started` and `meeting.ended` update the mapped durable `live_rooms.is_active` row before Redis state publication through the verified update policy. Transient mapping/state failures return `503` without consuming the delivery ID so provider retries remain safe. Staging abuse proof requires `ABUSE_LIMIT_ZOOM_WEBHOOK_REQUESTS` and `ABUSE_LIMIT_ZOOM_WEBHOOK_WINDOW_SECONDS`.
