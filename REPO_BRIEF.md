...
- AI and Zoom paths fail closed/degrade with explicit typed errors and auditability; AI chat/embedding calls use bounded shared timeout/retry policy, request-body replay, cancellation-aware backoff, and a 1 MiB provider-response cap. AI generation also requires its core dependencies and audit persistence; missing configuration or audit writes return typed `503` faults instead of serving unaudited output.
